    ...
    $.fn.scrollPagination.loadContent = function(obj, opts){
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                if(!data.error)
                {
                    if(data.curentPage == data.totalPages){
                        $.fn.stopScrollPagination(obj, opts);
                    }
                    else{
                        opts.data.page++;
    ...
    $.fn.scrollPagination.init = function(obj, opts){
        ...
        if(opts.autoload === true){
            $(target).scroll(function(event){
                if($(obj).attr('scrollPagination') == 'enabled' && $(obj).attr('loadingData') != 'waiting'){ 
                                    
                    //var mayLoadContent = (Math.ceil($(target).scrollTop())  + $(target).innerHeight()) >= ($(target)[0].scrollHeight );
                    if($(target).scrollTop() == 0){
                        $(obj).attr('loadingData', 'waiting');
                        $.fn.scrollPagination.loadContent(obj, opts);
                    }
                }else{
